...
public sealed class ProductTests
{
    ...
    {
        var product = Product.Create(" PRD-1 ", " 12345678 ", " Cement ", 2, 0.444m);

        Assert.Equal("PRD-1", product.Code);
    ...
    public void Create_EmptyName_Throws(string? name)
        => Assert.Throws<ArgumentException>(() =>
            Product.Create("PRD-1", null, name!, 1, 1m));

    [Fact]
    public void Create_InvalidMeasurementUnit_Throws()
        => Assert.Throws<ArgumentOutOfRangeException>(() =>
            Product.Create("PRD-1", null, "Product", 0, 1m));

    [Fact]
    public void Create_NegativePrice_Throws()
        => Assert.Throws<ArgumentOutOfRangeException>(() =>
            Product.Create("PRD-1", null, "Product", 1, -0.001m));
}
