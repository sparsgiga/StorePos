namespace StorePos.Domain.Common;

public static class FinancialPrecision
{
    public const int MoneyScale = 2;
    public const int QuantityScale = 5;
    public const int UnitPriceScale = 5;

    public const decimal MaximumMoneyValue = 9999999999999999.99m;
    public const decimal MaximumFiveScaleValue = 9999999999999.99999m;

    public static decimal RoundMoney(decimal value)
        => decimal.Round(value, MoneyScale, MidpointRounding.AwayFromZero);

    public static decimal RoundQuantity(decimal value)
        => decimal.Round(value, QuantityScale, MidpointRounding.AwayFromZero);

    public static decimal RoundUnitPrice(decimal value)
        => decimal.Round(value, UnitPriceScale, MidpointRounding.AwayFromZero);

    public static decimal CalculateLineTotal(decimal quantity, decimal unitPrice)
    {
        var normalizedQuantity = RoundQuantity(quantity);
        var normalizedUnitPrice = RoundUnitPrice(unitPrice);

        decimal product;
        try
        {
            product = checked(normalizedQuantity * normalizedUnitPrice);
        }
        catch (OverflowException)
        {
            throw new ArgumentOutOfRangeException(
                nameof(unitPrice),
                "Quantity multiplied by unit price is too large.");
        }

        var lineTotal = RoundMoney(product);
        if (lineTotal < 0.01m)
        {
            throw new ArgumentOutOfRangeException(
                nameof(unitPrice),
                "The final line total must be at least 0.01 GEL.");
        }

        if (lineTotal > MaximumMoneyValue)
        {
            throw new ArgumentOutOfRangeException(
                nameof(unitPrice),
                "The final line total is too large.");
        }

        return lineTotal;
    }

    public static decimal SumMoney(IEnumerable<decimal> values)
    {
        ArgumentNullException.ThrowIfNull(values);

        var total = 0m;
        try
        {
            foreach (var value in values)
            {
                total = checked(total + RoundMoney(value));
            }
        }
        catch (OverflowException exception)
        {
            throw new InvalidOperationException("The monetary total is too large.", exception);
        }

        total = RoundMoney(total);
        if (total > MaximumMoneyValue || total < -MaximumMoneyValue)
        {
            throw new InvalidOperationException("The monetary total is too large.");
        }

        return total;
    }
}
